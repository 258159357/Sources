...
namespace Terraria.Social.Steam
{
  public abstract class NetSocialModule : Terraria.Social.Base.NetSocialModule
  {
    ...
    public override void Initialize()
    {
      ...
      CoreSocialModule.OnTick += new Action((object) this._reader, __methodptr(ReadTick));
      // ISSUE: method pointer
      CoreSocialModule.OnTick += new Action((object) this._writer, __methodptr(SendAll));
    ...
    public override bool IsConnected(RemoteAddress address)
    {
      ...
      if (!this._connectionStateMap.ContainsKey(steamId) || this._connectionStateMap.get_Item(steamId) != NetSocialModule.ConnectionState.Connected)
        return false;
