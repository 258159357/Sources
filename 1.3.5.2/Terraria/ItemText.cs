  ...
  public class ItemText
  {
    ...
    private void ValueToName()
    {
      ...
      if (num1 > 0)
      {
        ...
        string str = itemText.name + (object) num1 + string.Format(" {0} ", (object) Language.GetTextValue("Currency.Platinum"));
        itemText.name = str;
      }
      if (num2 > 0)
      {
        ItemText itemText = this;
        string str = itemText.name + (object) num2 + string.Format(" {0} ", (object) Language.GetTextValue("Currency.Gold"));
        itemText.name = str;
      }
      if (num3 > 0)
      {
        ItemText itemText = this;
        string str = itemText.name + (object) num3 + string.Format(" {0} ", (object) Language.GetTextValue("Currency.Silver"));
        itemText.name = str;
      }
      if (num4 > 0)
      {
        ItemText itemText = this;
        string str = itemText.name + (object) num4 + string.Format(" {0} ", (object) Language.GetTextValue("Currency.Copper"));
        itemText.name = str;
      }
