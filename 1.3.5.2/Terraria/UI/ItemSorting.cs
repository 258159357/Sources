...
namespace Terraria.UI
{
  public class ItemSorting
  {
    ...
    private class ItemSortingLayers
    {
      ...
      {
        List<int> list = itemsToSort.Where<int>((Func<int, bool>) (i =>
        {
          if (inv[i].maxStack == 1 && inv[i].damage > 0 && (inv[i].ammo == 0 && inv[i].melee) && (inv[i].pick < 1 && inv[i].hammer < 1))
            return inv[i].axe < 1;
          return false;
        })).ToList<int>();
      ...
      {
        List<int> list = itemsToSort.Where<int>((Func<int, bool>) (i => (int) inv[i].dye > 0)).ToList<int>();
        layer.Validate(ref list, inv);
        foreach (int num in list)
          itemsToSort.Remove(num);
        list.Sort((Comparison<int>) ((x, y) =>
      ...
      {
        List<int> list = itemsToSort.Where<int>((Func<int, bool>) (i => (int) inv[i].hairDye >= 0)).ToList<int>();
        layer.Validate(ref list, inv);
        foreach (int num in list)
          itemsToSort.Remove(num);
        list.Sort((Comparison<int>) ((x, y) =>
