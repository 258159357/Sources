  ...
  public class ReflectiveArmorShaderData : ArmorShaderData
  {
    ...
    public override void Apply(Entity entity, DrawData? drawData)
    {
      ...
      else
      {
        ...
        Vector2 vector2 = position + new Vector2(width, height) * 0.1f;
        float x = width * 0.8f;
        float y = height * 0.8f;
        Vector3 subLight1 = Lighting.GetSubLight(vector2 + new Vector2(x * 0.5f, 0.0f));
        Vector3 subLight2 = Lighting.GetSubLight(vector2 + new Vector2(0.0f, y * 0.5f));
        Vector3 subLight3 = Lighting.GetSubLight(vector2 + new Vector2(x, y * 0.5f));
        Vector3 subLight4 = Lighting.GetSubLight(vector2 + new Vector2(x * 0.5f, y));
