    ...
    private void Load(string path, bool cloud)
    {
      ...
      lock (AchievementManager._ioLock)
      {
        if (!FileUtilities.Exists(path, cloud))
          return;
        byte[] local_3 = FileUtilities.ReadAllBytes(path, cloud);
        Dictionary<string, AchievementManager.StoredAchievement> local_4 = (Dictionary<string, AchievementManager.StoredAchievement>) null;
        try
        {
          using (MemoryStream resource_2 = new MemoryStream(local_3))
          {
            using (CryptoStream resource_1 = new CryptoStream((Stream) resource_2, new RijndaelManaged().CreateDecryptor(this._cryptoKey, this._cryptoKey), CryptoStreamMode.Read))
            {
              using (BsonReader resource_0 = new BsonReader((Stream) resource_1))
                local_4 = (Dictionary<string, AchievementManager.StoredAchievement>) JsonSerializer.Create(this._serializerSettings).Deserialize<Dictionary<string, AchievementManager.StoredAchievement>>((JsonReader) resource_0);
            }
          }
        }
        catch (Exception exception_0)
        ...
        if (local_4 == null)
          return;
        foreach (KeyValuePair<string, AchievementManager.StoredAchievement> item_0 in local_4)
        {
