...
namespace Terraria.IO
{
  public class WorldFile
  {
      ...
      using (MemoryStream memoryStream = new MemoryStream(FileUtilities.ReadAllBytes(Main.worldPathName, flag)))
      {
        using (BinaryReader binaryReader = new BinaryReader((Stream) memoryStream))
        {
          try
          {
            ...
            int num2 = 87;
            int num3 = num1 > num2 ? WorldFile.LoadWorld_Version2(binaryReader) : WorldFile.LoadWorld_Version1(binaryReader);
            int num4 = 141;
            if (num1 < num4)
            ...
            while (Liquid.numLiquid > 0 && num5 < 100000)
            {
              ...
              if ((double) num8 > (double) num7)
                ...
                num8 = num7;
              Main.statusText = Lang.gen[27].Value + " " + (object) (int) ((double) num8 * 100.0 / 2.0 + 50.0) + "%";
              Liquid.UpdateLiquid();
            }
    ...
    public static void saveWorld(bool useCloudSaving, bool resetTime = false)
    {
      ...
      lock (WorldFile.padlock)
      {
        ...
        if (Main.worldPathName == null)
          return;
        new Stopwatch().Start();
        byte[] local_2 = (byte[]) null;
        int local_3 = 0;
        using (MemoryStream resource_1 = new MemoryStream(7000000))
        {
          using (BinaryWriter resource_0 = new BinaryWriter((Stream) resource_1))
            WorldFile.SaveWorld_Version2(resource_0);
          local_2 = resource_1.ToArray();
          local_3 = local_2.Length;
        }
        if (local_2 == null)
          return;
        byte[] local_4 = (byte[]) null;
        if (FileUtilities.Exists(Main.worldPathName, useCloudSaving))
          local_4 = FileUtilities.ReadAllBytes(Main.worldPathName, useCloudSaving);
        FileUtilities.Write(Main.worldPathName, local_2, local_3, useCloudSaving);
        byte[] local_2_1 = FileUtilities.ReadAllBytes(Main.worldPathName, useCloudSaving);
        string local_5 = (string) null;
        using (MemoryStream resource_3 = new MemoryStream(local_2_1, 0, local_3, false))
        {
          using (BinaryReader resource_2 = new BinaryReader((Stream) resource_3))
          {
            if (!Main.validateSaves || WorldFile.validateWorld(resource_2))
            {
              if (local_4 != null)
              {
                local_5 = Main.worldPathName + ".bak";
                Main.statusText = Lang.gen[50].Value;
              }
            }
            else
              local_5 = Main.worldPathName;
          }
        }
        if (local_5 != null && local_4 != null)
          FileUtilities.WriteAllBytes(local_5, local_4, useCloudSaving);
    ...
    public static int LoadWorld_Version1(BinaryReader fileIO)
    {
        ...
        for (int index2 = 0; index2 < Main.maxTilesY; ++index2)
        {
          ...
          if (tile.active())
          {
            ...
            if ((int) tile.type == (int) sbyte.MaxValue)
              tile.active(false);
            if (versionNumber < 72 && ((int) tile.type == 35 || (int) tile.type == 36 || ((int) tile.type == 170 || (int) tile.type == 171) || (int) tile.type == 172))
            {
            ...
            else if (Main.tileFrameImportant[index3])
            {
              ...
              else if (versionNumber < 40 && (int) tile.type == 19)
              {
                tile.frameX = (short) 0;
                tile.frameY = (short) 0;
              }
      ...
      for (int index1 = 0; index1 < 1000; ++index1)
      {
        if (fileIO.ReadBoolean())
        {
          ...
          Main.chest[index1].x = fileIO.ReadInt32();
          ...
          for (int index2 = 0; index2 < 40; ++index2)
          {
            ...
            if (index2 < num5)
            {
              ...
              if (num1 > 0)
              {
                if (versionNumber >= 38)
                {
                  Main.chest[index1].item[index2].netDefaults(fileIO.ReadInt32());
                }
                else
                {
                ...
                Main.chest[index1].item[index2].stack = num1;
                if (versionNumber >= 36)
      ...
      while (flag)
      {
        if (versionNumber >= 190)
          Main.npc[index].SetDefaults(fileIO.ReadInt32(), -1f);
        else
      ...
      if (versionNumber < 7)
        return 0;
      int num6 = fileIO.ReadBoolean() ? 1 : 0;
      string str1 = fileIO.ReadString();
      int num7 = fileIO.ReadInt32();
      return num6 != 0 && (str1 == Main.worldName || num7 == Main.worldID) ? 0 : 2;
    }
    ...
    private static int SaveWorldTiles(BinaryWriter writer)
    {
      ...
      for (int i = 0; i < Main.maxTilesX; ++i)
      {
        ...
        for (int j = 0; j < Main.maxTilesY; j = num2 + 1)
        {
          ...
          short num9 = 0;
          int index6 = j + 1;
          for (int index2 = Main.maxTilesY - j - 1; index2 > 0 && tile.isTheSameAs(Main.tile[i, index6]); ++index6)
          {
            ++num9;
            --index2;
          }
      ...
      for (int index1 = 0; index1 < Main.maxTilesX; ++index1)
      {
        ...
        for (int index2 = 0; index2 < Main.maxTilesY; ++index2)
        {
          ...
          if ((int) num3 > 1)
          {
            ...
            if ((int) num5 != 0 && Main.tileSolid[(int) from.type])
            {
              if ((int) num5 == 1)
                from.halfBrick(true);
              else
          ...
          switch ((byte) (((int) num4 & 192) >> 6))
          {
            case 0:
              ...
              break;
            case 1:
              num7 = (int) reader.ReadByte();
              break;
            default:
              num7 = (int) reader.ReadInt16();
              break;
          }
          if (index3 != -1)
          {
    ...
    {
      return !reader.ReadBoolean() || reader.ReadString() != Main.worldName || reader.ReadInt32() != Main.worldID ? 6 : 0;
    }
      ...
      try
      {
        ...
        for (int index1 = 0; index1 < num5; ++index1)
        {
          ...
          for (int index2 = 0; index2 < num4; index2 = index2 + num6 + 1)
          {
            ...
            if (((int) num8 & 1) == 1 && ((int) fileIO.ReadByte() & 1) == 1)
            ...
            if (((int) num8 & 2) == 2)
            {
              ...
              if (((int) num8 & 32) == 32)
              {
                ...
                index3 = (int) fileIO.ReadByte() << 8 | (int) num9;
              }
              else
        ...
        int num16 = fileIO.ReadBoolean() ? 1 : 0;
        string str2 = fileIO.ReadString();
        int num17 = fileIO.ReadInt32();
        bool flag1 = false;
        if (num16 != 0 && (str2 == str1 || num17 == num3))
          flag1 = true;
        return flag1;
      }
      catch (Exception ex)
    ...
    public static void FixDresserChests()
    {
      for (int X = 0; X < Main.maxTilesX; ++X)
      {
        ...
        {
          Tile tile = Main.tile[X, Y];
          if (tile.active() && (int) tile.type == 88 && ((int) tile.frameX % 54 == 0 && (int) tile.frameY % 36 == 0))
    ...
    private static void LoadTileEntities(BinaryReader reader)
    {
      ...
      foreach (KeyValuePair<Point16, TileEntity> keyValuePair in TileEntity.ByPosition)
      {
        ...
        {
          if ((int) keyValuePair.Value.type == 0 && !TETrainingDummy.ValidTile((int) keyValuePair.Value.Position.X, (int) keyValuePair.Value.Position.Y))
            point16List.Add(keyValuePair.Value.Position);
          if ((int) keyValuePair.Value.type == 2 && !TELogicSensor.ValidTile((int) keyValuePair.Value.Position.X, (int) keyValuePair.Value.Position.Y))
            point16List.Add(keyValuePair.Value.Position);
          if ((int) keyValuePair.Value.type == 1 && !TEItemFrame.ValidTile((int) keyValuePair.Value.Position.X, (int) keyValuePair.Value.Position.Y))
            point16List.Add(keyValuePair.Value.Position);
        }
      }
      try
