  ...
  public class WaterShaderData : ScreenShaderData
  {
    ...
    public override void Update(GameTime gameTime)
    {
      ...
      if (Main.gamePaused || !Main.hasFocus)
        return;
      this._progress = this._progress + (float) (gameTime.ElapsedGameTime.TotalSeconds * (double) this.Intensity * 0.75);
      ...
      if (this._useProjectileWaves)
      {
        for (int index = 0; index < 1000; ++index)
        {
          ...
          if (((projectile == null || !projectile.active ? 0 : (ProjectileID.Sets.CanDistortWater[projectile.type] ? 1 : 0)) & (flag3 ? 1 : 0)) != 0 && !ProjectileID.Sets.NoLiquidDistortion[projectile.type] && Collision.CheckAABBvAABBCollision(screenPosition, dimensions1, projectile.position - vector2_2, projectile.Size + vector2_2))
          {
            ...
            float num4 = 2f * (float) Math.Sqrt(0.0500000007450581 * (double) projectile.velocity.Length());
            ...
            tileBatch.Draw(Main.magicPixel, new Vector4(vector2_3.X - z * 0.5f, vector2_3.Y - w * 0.5f, z, w) * 0.25f, new VertexColors(new Color((float) ((double) velocity.X * 0.5 + 0.5), (float) ((double) velocity.Y * 0.5 + 0.5), num4 * 0.5f)));
          }
        }
      }
      tileBatch.End();
    ...
    {
      private static readonly Rectangle[] RIPPLE_SHAPE_SOURCE_RECTS = new Rectangle[3]{ new Rectangle(0, 0, 0, 0), new Rectangle(1, 1, 62, 62), new Rectangle(1, 65, 62, 62) };
