...
namespace Terraria.GameContent.UI
{
  public class CustomCurrencySingleCoin : CustomCurrencySystem
  {
    ...
    public override void DrawSavingsMoney(SpriteBatch sb, string text, float shopx, float shopy, long totalCoins, bool horizontal = false)
    {
      ...
      if (horizontal)
      {
        Vector2 position = new Vector2((float) ((double) shopx + (double) ChatManager.GetStringSize(Main.fontMouseText, text, Vector2.One, -1f).X + 45.0), shopy + 50f);
    ...
    public override void GetPriceText(string[] lines, ref int currentLine, int price)
    {
      ...
      int& local = @currentLine;
      int num1 = currentLine;
      int num2 = num1 + 1;
      // ISSUE: explicit reference operation
      ^local = num2;
      int index = num1;
      string str = string.Format("[c/{0:X2}{1:X2}{2:X2}:{3} {4} {5}]", (object) color.R, (object) color.G, (object) color.B, (object) Lang.tip[50].Value, (object) price, (object) Language.GetTextValue(this.CurrencyTextKey).ToLower());
      strArray[index] = str;
    }
  }
}
