...
namespace Terraria.GameInput
{
  public class PlayerInput
  {
    ...
    private static void GamePadInput()
    {
      ...
      if (!PlayerInput.CursorIsBusy)
      {
        ...
        if (gamepadThumbstickLeft != Vector2.Zero & flag7)
        {
          ...
          {
            float m11 = Main.GameViewMatrix.ZoomMatrix.M11;
            Vector2 vector2_3 = gamepadThumbstickLeft * new Vector2((float) (Player.tileRangeX * 16), (float) (Player.tileRangeY * 16)) * m11;
            if (num3 != 0)
              vector2_3 = gamepadThumbstickLeft * new Vector2((float) ((Player.tileRangeX + num3) * 16), (float) ((Player.tileRangeY + num3) * 16)) * m11;
            if (flag4)
    ...
    public static string ComposeInstructionsForGamepad()
    {
      ...
      {
        str2 = str1 + "          " + PlayerInput.BuildCommand(Lang.misc[56].Value, 0 != 0, PlayerInput.ProfileGamepadUI.KeyStatus["Inventory"]) + PlayerInput.BuildCommand(Lang.inter[118].Value, 0 != 0, PlayerInput.ProfileGamepadUI.KeyStatus["HotbarMinus"]) + PlayerInput.BuildCommand(Lang.inter[119].Value, 0 != 0, PlayerInput.ProfileGamepadUI.KeyStatus["HotbarPlus"]);
        if (Main.netMode == 1 && Main.player[Main.myPlayer].HasItem(2997))
          str2 += PlayerInput.BuildCommand(Lang.inter[120].Value, 0 != 0, PlayerInput.ProfileGamepadUI.KeyStatus["MouseRight"]);
      }
      ...
      else
      {
        if (!PlayerInput.GrappleAndInteractAreShared || !WiresUI.Settings.DrawToolModeUI && (!Main.SmartInteractShowingGenuine || Main.SmartInteractNPC == -1 && (Main.SmartInteractX == -1 || Main.SmartInteractY == -1)))
          str1 += PlayerInput.BuildCommand(Lang.misc[57].Value, 0 != 0, inputMode.KeyStatus["Grapple"]);
        string str3 = str1 + PlayerInput.BuildCommand(Lang.misc[58].Value, 0 != 0, inputMode.KeyStatus["Jump"]) + PlayerInput.BuildCommand(Lang.misc[59].Value, 0 != 0, inputMode.KeyStatus["HotbarMinus"], inputMode.KeyStatus["HotbarPlus"]);
        if (PlayerInput.InBuildingMode)
          str3 += PlayerInput.BuildCommand(Lang.menu[6].Value, 0 != 0, inputMode.KeyStatus["Inventory"], inputMode.KeyStatus["MouseRight"]);
        if (WiresUI.Open)
        {
          str2 = str3 + PlayerInput.BuildCommand(Lang.misc[53].Value, 0 != 0, inputMode.KeyStatus["MouseLeft"]) + PlayerInput.BuildCommand(Lang.misc[56].Value, 0 != 0, inputMode.KeyStatus["MouseRight"]);
        }
        else
        {
          Item obj = Main.player[Main.myPlayer].inventory[Main.player[Main.myPlayer].selectedItem];
          if (obj.damage > 0 && obj.ammo == 0)
            str2 = str3 + PlayerInput.BuildCommand(Lang.misc[60].Value, 0 != 0, inputMode.KeyStatus["MouseLeft"]);
          else if (obj.createTile >= 0 || obj.createWall > 0)
            str2 = str3 + PlayerInput.BuildCommand(Lang.misc[61].Value, 0 != 0, inputMode.KeyStatus["MouseLeft"]);
          else
            str2 = str3 + PlayerInput.BuildCommand(Lang.misc[63].Value, 0 != 0, inputMode.KeyStatus["MouseLeft"]);
          if (Main.SmartInteractShowingGenuine)
          {
            if (Main.SmartInteractNPC != -1)
              str2 += PlayerInput.BuildCommand(Lang.misc[80].Value, 0 != 0, inputMode.KeyStatus["MouseRight"]);
            else if (Main.SmartInteractX != -1 && Main.SmartInteractY != -1)
            {
              ...
              if (TileID.Sets.TileInteractRead[(int) tile.type])
                str2 += PlayerInput.BuildCommand(Lang.misc[81].Value, 0 != 0, inputMode.KeyStatus["MouseRight"]);
              else
                str2 += PlayerInput.BuildCommand(Lang.misc[79].Value, 0 != 0, inputMode.KeyStatus["MouseRight"]);
            }
          }
          else if (WiresUI.Settings.DrawToolModeUI)
            str2 += PlayerInput.BuildCommand(Lang.misc[89].Value, 0 != 0, inputMode.KeyStatus["MouseRight"]);
        }
      }
      return str2;
    }
