...
namespace Terraria
{
  public class Collision
  {
    ...
    private static double DistFromSeg(Vector2 p, Vector2 q0, Vector2 q1, double radius, ref float u)
    {
      ...
      double num6 = num4;
      return Math.Abs(num1 * num6 - num3 * num2) / num5;
    }
    ...
    public static bool CheckAABBvLineCollision(Vector2 objectPosition, Vector2 objectDimensions, Vector2 lineStart, Vector2 lineEnd, float lineWidth, ref float collisionPoint)
    {
      ...
      for (int index1 = 0; index1 < vector2Array.Length; ++index1)
      {
        ...
        if ((double) num5 != 0.0)
        {
          ...
          if ((double) num3 >= 0.0 && (double) num3 <= 1.0)
          {
            float num4 = (float) ((double) vector2_8.X * (double) vector2_9.Y - (double) vector2_8.Y * (double) vector2_9.X) / num5;
            if ((double) num4 >= 0.0 && (double) num4 <= 1.0)
            {
              flag = true;
              collisionPoint = Math.Min(collisionPoint, vector2_4.X + num3 * vector2_9.X);
            }
          }
        }
      }
    ...
    public static Vector4 SlopeCollision(Vector2 Position, Vector2 Velocity, int Width, int Height, float gravity = 0.0f, bool fall = false)
    {
      ...
      int num13 = 0;
      int gravDir = 1;
      Vector2 vector2_6 = Collision.TileCollision(Position1, Velocity1, Width1, Height1, num12 != 0, num13 != 0, gravDir);
      if ((double) vector2_6.Y > (double) vector2_5.Y)
      {
        float num9 = vector2_5.Y - vector2_6.Y;
      ...
      {
        float num9 = vector2_6.Y - vector2_5.Y;
        vector2_2.Y = Position.Y + vector2_6.Y;
        if (flagArray[3])
          vector2_2.X = Position.X - num9;
        if (flagArray[4])
    ...
    public static Vector2 noSlopeCollision(Vector2 Position, Vector2 Velocity, int Width, int Height, bool fallThrough = false, bool fall2 = false)
    {
      ...
      int min = 0;
      int max = Main.maxTilesX - 1;
      int num9 = Utils.Clamp<int>(num1, min, max);
    ...
    public static List<Point> FindCollisionTile(int Direction, Vector2 position, float testMagnitude, int Width, int Height, bool fallThrough = false, bool fall2 = false, int gravDir = 1, bool checkCardinals = true, bool checkSlopes = false)
    {
      ...
      if ((uint) Direction > 1U)
      {
        ...
        {
          Vector2 Velocity = Direction == 2 ? Vector2.UnitY * testMagnitude : -Vector2.UnitY * testMagnitude;
          Vector4 vec = new Vector4(position, Velocity.X, Velocity.Y);
          int y = (int) ((double) position.Y + (Direction == 2 ? (double) Height : 0.0)) / 16;
          float num1 = Math.Min((float) (16.0 - (double) position.X % 16.0), (float) Width);
          float num2 = num1;
          if (checkCardinals && Collision.TileCollision(position - Velocity, Velocity, (int) num1, Height, fallThrough, fall2, gravDir) != Velocity)
            pointList.Add(new Point((int) position.X / 16, y));
          else if (checkSlopes && Collision.SlopeCollision(position, Velocity, (int) num1, Height, (float) gravDir, fallThrough).YZW() != vec.YZW())
            pointList.Add(new Point((int) position.X / 16, y));
          while ((double) num2 + 16.0 <= (double) (Width - 16))
          {
            if (checkCardinals && Collision.TileCollision(position - Velocity + Vector2.UnitX * num2, Velocity, 16, Height, fallThrough, fall2, gravDir) != Velocity)
              pointList.Add(new Point((int) ((double) position.X + (double) num2) / 16, y));
            else if (checkSlopes && Collision.SlopeCollision(position + Vector2.UnitX * num2, Velocity, 16, Height, (float) gravDir, fallThrough).YZW() != vec.YZW())
              pointList.Add(new Point((int) ((double) position.X + (double) num2) / 16, y));
            num2 += 16f;
          }
          int Width1 = Width - (int) num2;
          if (checkCardinals && Collision.TileCollision(position - Velocity + Vector2.UnitX * num2, Velocity, Width1, Height, fallThrough, fall2, gravDir) != Velocity)
            pointList.Add(new Point((int) ((double) position.X + (double) num2) / 16, y));
          else if (checkSlopes && Collision.SlopeCollision(position + Vector2.UnitX * num2, Velocity, Width1, Height, (float) gravDir, fallThrough).YZW() != vec.YZW())
            pointList.Add(new Point((int) ((double) position.X + (double) num2) / 16, y));
        }
      }
      else
      {
        Vector2 Velocity = Direction == 0 ? Vector2.UnitX * testMagnitude : -Vector2.UnitX * testMagnitude;
        Vector4 vec = new Vector4(position, Velocity.X, Velocity.Y);
        ...
        float num2 = num1;
        if (checkCardinals && Collision.TileCollision(position - Velocity, Velocity, Width, (int) num1, fallThrough, fall2, gravDir) != Velocity)
          pointList.Add(new Point(x, (int) position.Y / 16));
        else if (checkSlopes && Collision.SlopeCollision(position, Velocity, Width, (int) num1, (float) gravDir, fallThrough).XZW() != vec.XZW())
          pointList.Add(new Point(x, (int) position.Y / 16));
        while ((double) num2 + 16.0 <= (double) (Height - 16))
        {
          if (checkCardinals && Collision.TileCollision(position - Velocity + Vector2.UnitY * num2, Velocity, Width, 16, fallThrough, fall2, gravDir) != Velocity)
            pointList.Add(new Point(x, (int) ((double) position.Y + (double) num2) / 16));
          else if (checkSlopes && Collision.SlopeCollision(position + Vector2.UnitY * num2, Velocity, Width, 16, (float) gravDir, fallThrough).XZW() != vec.XZW())
            pointList.Add(new Point(x, (int) ((double) position.Y + (double) num2) / 16));
          num2 += 16f;
        }
        int Height1 = Height - (int) num2;
        if (checkCardinals && Collision.TileCollision(position - Velocity + Vector2.UnitY * num2, Velocity, Width, Height1, fallThrough, fall2, gravDir) != Velocity)
          pointList.Add(new Point(x, (int) ((double) position.Y + (double) num2) / 16));
        else if (checkSlopes && Collision.SlopeCollision(position + Vector2.UnitY * num2, Velocity, Width, Height1, (float) gravDir, fallThrough).XZW() != vec.XZW())
          pointList.Add(new Point(x, (int) ((double) position.Y + (double) num2) / 16));
      }
      return pointList;
    }
    ...
    public static void StepDown(ref Vector2 position, ref Vector2 velocity, int width, int height, ref float stepSpeed, ref float gfxOffY, int gravDir = 1, bool waterWalk = false)
    {
      ...
      if ((double) num9 > 9.0)
        stepSpeed = 2.5f;
      gfxOffY = gfxOffY + (position.Y + (float) height - num5);
    ...
    public static void StepUp(ref Vector2 position, ref Vector2 velocity, int width, int height, ref float stepSpeed, ref float gfxOffY, int gravDir = 1, bool holdsMatching = false, int specialChecksMode = 0)
    {
      ...
      if (gravDir == 1)
      {
        ...
        flag8 = ((!flag5 ? (false ? 1 : 0) : (!tile4.nactive() || tile4.topSlope() && ((int) tile4.slope() != 1 || (double) position.X + (double) (width / 2) >= (double) (x * 16)) && ((int) tile4.slope() != 2 || (double) position.X + (double) (width / 2) <= (double) (x * 16 + 16)) || tile4.topSlope() && (double) position.Y + (double) height <= (double) (index1 * 16) || (!Main.tileSolid[(int) tile4.type] || Main.tileSolidTop[(int) tile4.type]) && ((!holdsMatching || (!Main.tileSolidTop[(int) tile4.type] || (int) tile4.frameY != 0) && !TileID.Sets.Platforms[(int) tile4.type] ? 0 : (!Main.tileSolid[(int) tile5.type] ? 1 : (!tile5.nactive() ? 1 : 0))) & (flag6 ? 1 : 0)) == 0 ? (!tile5.halfBrick() ? (false ? 1 : 0) : (tile5.nactive() ? 1 : 0)) : (true ? 1 : 0))) & (!Main.tileSolidTop[(int) tile4.type] ? 1 : (!Main.tileSolidTop[(int) tile5.type] ? 1 : 0))) != 0;
      }
      else
      {
        Tile tile2 = Main.tile[x, index1 - gravDir];
      ...
      if (gravDir == 1)
      {
        ...
        if ((double) num4 > 16.1)
          return;
        gfxOffY = gfxOffY + (position.Y + (float) height - num3);
      ...
      else
      {
        ...
        if ((double) num4 > 16.1)
          return;
        gfxOffY = gfxOffY - (num3 - position.Y);
        position.Y = num3;
        velocity.Y = 0.0f;
        if ((double) num4 < 9.0)
          stepSpeed = 1f;
    ...
    public static float GetTileRotation(Vector2 position)
    {
      ...
      for (int index3 = 2; index3 >= 0; --index3)
      {
        if (tile.active())
        {
          ...
          else if (((!Main.tileSolidTop[(int) tile.type] ? 0 : ((int) tile.frameY == 0 ? 1 : 0)) & (flag ? 1 : 0)) != 0)
            return 0.0f;
        }
        ++index2;
    ...
    public static void LaserScan(Vector2 samplingPoint, Vector2 directionUnit, float samplingWidth, float maxDistance, float[] samples)
    {
      for (int index = 0; index < samples.Length; ++index)
      {
        ...
        double radians = 1.57079637050629;
        Vector2 vector2_2 = new Vector2();
        Vector2 center = vector2_2;
        Vector2 vector2_3 = spinningpoint.RotatedBy(radians, center) * (num1 - 0.5f) * samplingWidth;
        Vector2 vector2_4 = vector2_1 + vector2_3;
        int x1 = (int) vector2_4.X / 16;
        int y1 = (int) vector2_4.Y / 16;
        Vector2 vector2_5 = directionUnit * maxDistance;
        Vector2 vector2_6 = vector2_4 + vector2_5;
        int x2 = (int) vector2_6.X / 16;
        int y2 = (int) vector2_6.Y / 16;
        Tuple<int, int> col;
        float num2;
