...
namespace Terraria.IO
{
  public class WorldFile
  {
    ...
    public static event Action OnWorldLoad
    {
      add
      {
        Action action1 = WorldFile.OnWorldLoad;
        Action comparand;
        do
        {
          comparand = action1;
          Action action2 = (Action) Delegate.Combine((Delegate) comparand, (Delegate) value);
          action1 = Interlocked.CompareExchange<Action>(ref WorldFile.OnWorldLoad, action2, comparand);
        }
    ...
    public static void saveWorld(bool useCloudSaving, bool resetTime = false)
    {
      ...
      try
      {
        Monitor.Enter(padlock = WorldFile.padlock, ref flag);
        try
        {
