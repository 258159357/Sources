...
namespace Terraria.GameContent.Tile_Entities
{
  public class TELogicSensor : TileEntity
  {
    ...
    public static void Initialize()
    {
      ...
      TileEntity._UpdateStart += new Action((object) null, __methodptr(UpdateStartInternal));
      // ISSUE: method pointer
      TileEntity._UpdateEnd += new Action((object) null, __methodptr(UpdateEndInternal));
      // ISSUE: method pointer
      TileEntity._NetPlaceEntity += new Action<int, int, int>((object) null, __methodptr(NetPlaceEntity));
    }
    ...
    public void ChangeState(bool onState, bool TripWire)
    {
      ...
      if (!TripWire || Main.netMode == 1)
        return;
      TELogicSensor.tripPoints.Add((Tuple<Point16, bool>) Tuple.Create<Point16, bool>((M0) this.Position, (M1) (this.logicCheck == TELogicSensor.LogicCheckType.PlayerAbove ? 1 : 0)));
    }

    public static bool ValidTile(int x, int y)
    {
      return Main.tile[x, y].active() && (int) Main.tile[x, y].type == 423 && ((int) Main.tile[x, y].frameY % 18 == 0 && (int) Main.tile[x, y].frameX % 18 == 0);
    }
    ...
    {
      if (Main.tile[x, y].active() && (int) Main.tile[x, y].type == 423)
        return true;
    ...
    {
      TileEntity tileEntity;
      if (!TileEntity.ByPosition.TryGetValue(new Point16(x, y), out tileEntity) || (int) tileEntity.type != 2)
        return;
    ...
    {
      return ((int) this.Position.X).ToString() + "x  " + (object) this.Position.Y + "y " + (object) this.logicCheck;
    }
